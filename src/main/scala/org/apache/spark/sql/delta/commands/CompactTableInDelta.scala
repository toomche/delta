...
case class CompactTableInDelta(
  ...
  protected def optimize(sparkSession: SparkSession,
    ...
    if (readVersion < 0) {
      ...
      DeltaErrors.notADeltaTableException("compact",
        new DeltaTableIdentifier(Option(deltaLog.dataPath.toString), None))
    }
    ...
    def writeFiles(outputPath: Path,
      ...
      SQLExecution.withNewExecutionId(spark, queryExecution) {
        ...
        FileFormatWriter.write(
          ...
          // Here, we directly write into this outputPath without partition
    ...
    filesShouldBeOptimized.foreach { fileList =>
      ...
      if (tempFiles.length >= compactNumFilePerDir) {
        ...
        newFiles ++= writeFiles(filePath, df, Some(options), false).map { addFile =>
          addFile.copy(path = PathFun(prefix)
            .add(addFile.path).toPath.stripPrefix("/"), partitionValues = partitionValues)
        }
...
object PathFun {
  ...
  def joinPath(rootPath: String, paths: String*): String = {
